...
const MethodologyCards: React.FC = () => {
  return (
    <section id="methodologies" className="section-padding bg-zinc-50">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {methodologies.map((method, index) => (
            <RevealAnimation key={method.id} direction="scale" delay={index * 100}>
              <div className="h-full card-hover rounded-xl p-6 bg-white border border-zinc-100 shadow-sm transition-all hover:shadow flex flex-col">
                <div className={`w-12 h-12 rounded-lg flex items-center justify-center bg-${method.color}-100 text-${method.color}-600 mb-4`}>
                  {method.icon}
                </div>
